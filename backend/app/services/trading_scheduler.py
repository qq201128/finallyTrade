            ...
            try:
                with get_db_context() as db:
                    ...
                    if use_bidirectional:
                        # 使用双向交易引擎
                        logger.debug(f"策略 {user_strategy_id} 使用双向交易引擎")
                ...
                logger.error(f"策略 {user_strategy_id} 执行交易循环失败: {e}", exc_info=True)
                # 发生错误时等待一段时间再继续，避免快速重试
                time.sleep(10)
            
        except Exception as e:
            logger.error(f"策略 {user_strategy_id} 处理失败: {e}", exc_info=True)
